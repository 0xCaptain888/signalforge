...
def main():
    ...
    body = {"asset": "ETH", "candidate_signal": {
        "name": "t", "source": "cmc_fear_greed",
        "definition": "fg<20", "holding_period_days": 5}}
    ...
    demo = base64.b64encode(_json.dumps({
        "x402Version": 1, "network": "base",
        "payload": {"signature": "0xdemo", "authorization": {}}}).encode()).decode()
    r = client.post("/adjudicate", json=body, headers={"X-PAYMENT": demo})
    assert r.status_code == 200
    data = r.json()
    assert data["leakage_check"]["leaked"] is True
    assert data["edge_confidence"] == 12, f"Expected 12, got {data['edge_confidence']}"
    print(f"  [3/5] /adjudicate verdict={data['verdict']}, confidence=12 PASS")
