...
spec       -> 5cfa613c6e755fed…   (refreshed after Stage 2-7 second-pass audit:
                                   data_sources now carries per-endpoint
                                   plan-availability status + the Binance
                                   fallback is declared as a first-class source)
```
