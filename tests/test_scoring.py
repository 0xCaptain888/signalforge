...
class TestEdgeConfidence:
    ...
    def test_score_clamp_min(self):
        r = edge_confidence({
            "dsr_probability": 0.0, "fdr_significant_factors": 0,
            "walk_forward_median_sharpe": -5.0, "parameter_plateau": "spike",
            "lookahead_test": "FAIL", "strongest_t_stat": 0,
        })
        assert r.score >= 0

    def test_score_clamp_max(self):
        r = edge_confidence({
            "dsr_probability": 1.0, "fdr_significant_factors": 10,
            "walk_forward_median_sharpe": 3.0, "parameter_plateau": "plateau",
            "lookahead_test": "PASS", "strongest_t_stat": 10,
        })
        assert r.score <= 100
    ...
    def test_rule5_lookahead_fail_penalizes_30(self):
        base = {
            "dsr_probability": 0.80, "fdr_significant_factors": 2,
            "walk_forward_median_sharpe": 0.8, "parameter_plateau": "plateau",
            "strongest_t_stat": 0,
        }
    ...
    def test_plateau_vs_spike_20pt_swing(self):
        base = {
            "dsr_probability": 0.50, "fdr_significant_factors": 1,
            "walk_forward_median_sharpe": 0.5, "lookahead_test": "PASS",
            "strongest_t_stat": 0,
        }
    ...
    def test_rule6_regime_tstat_bands(self):
        base = {
            "dsr_probability": 0.50, "fdr_significant_factors": 0,
            "walk_forward_median_sharpe": 0.0, "parameter_plateau": "spike",
            "lookahead_test": "PASS",
        }
    ...
    def test_rule6_handles_none(self):
        r = edge_confidence({
            "dsr_probability": 0.5, "fdr_significant_factors": 0,
            "walk_forward_median_sharpe": 0.0, "parameter_plateau": "spike",
            "lookahead_test": "PASS", "strongest_t_stat": None,
        })
        assert r.breakdown["rule6_regime"] == 0

    def test_breakdown_keys(self, v1_stats):
        r = edge_confidence(v1_stats)
        for k in ["rule1_dsr", "rule2_fdr", "rule3_walkforward",
                  "rule4_plateau", "rule5_lookahead", "rule6_regime"]:
