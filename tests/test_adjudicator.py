...
import pytest
from src.adjudicator.core import (
    adjudicate,
    adjudicate_preset_leakage,
    adjudicate_preset_honest,
)
...
class TestAdjudicate:
    def test_returns_valid_schema(self):
        result = adjudicate(
            asset="ETH", candidate_signal=SAMPLE_SIGNAL, include_cmc_snapshot=False
        )
    ...
    def test_v1_real_data_verdict(self):
        ...
        assert r["edge_confidence"] < 60  # live pipeline yields 47

    def test_v1_edge_confidence_is_47(self):
        r = adjudicate_preset_leakage()
        assert r["edge_confidence"] == 47  # live pipeline value (not demo fallback 12)
    ...
    def test_leakage_check_fields(self):
        ...
        for f in [
            "leaked",
            "naive_sharpe_if_leaked",
            "honest_sharpe",
            "gap",
            "direction_flip_detected",
            "lookahead_flag",
        ]:
            assert f in lc
