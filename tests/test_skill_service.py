...
SAMPLE_BODY = {
    "asset": "ETH",
    "candidate_signal": {
        "name": "test", "source": "cmc_fear_greed",
...
class TestX402Gate:
    ...
    def test_adjudicate_v1_confidence_12(self, client):
        r = client.post("/adjudicate", json=SAMPLE_BODY,
                        headers={"X-PAYMENT": demo_auth_header()})
        assert r.json()["edge_confidence"] == 12

    def test_missing_signal_422(self, client):
        r = client.post("/adjudicate", json={"asset": "ETH"},
                        headers={"X-PAYMENT": demo_auth_header()})
    ...
    def test_generate_spec_with_payment(self, client):
        r = client.post("/generate_spec", json={"asset": "ETH", "risk": "balanced"},
                        headers={"X-PAYMENT": demo_auth_header()})
        assert r.status_code == 200
